...
public partial class CorrectionSlipLookupWindow : Window
{
    ...
    public CorrectionSlipLookupWindow(DatabaseService databaseService, string dataAreaId)
    {
        InitializeComponent();
    ...
    private async Task LoadLookupAsync()
    {
        try
        {
            ...
            var rows = await _databaseService.SearchCorrectableWeighmentsAsync(
                _dataAreaId,
            ...
            StatusTextBlock.Text = $"Loaded {LookupRows.Count:N0} completed transaction(s).";
        }
        catch (Exception ex)
        {
    ...
    private void SelectCurrentRow()
    {
        ...
        {
            StatusTextBlock.Text = "Please select a completed slip.";
            return;
        }
